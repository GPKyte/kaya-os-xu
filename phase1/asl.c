...
=======
	int *semAdd = s->semAdd;
>>>>>>> 040afe5f9483d00506850ef604cdea90b2995704
...
/*
 * return NULL if semdFree list is empty. Otherwise, remove an element from
 * the semdFree list, and then return a pointer to the removed element.
 * Note, it is up to the calling function to add the Semd to the ASL.
 */
...
int insertBlocked (int *semAdd, pcb_PTR p) {
	...
	p->s_semAdd = semAdd; /* set sema4 address of p to semAdd */
	semd_PTR predecessor = searchSemd(semAdd);
	/* Verify if sema4 already in place or need allocation */
 ...
 * otherwise, remove the fisrt (i.e. head) ProcBlk from the process queue of the found
...
pcb_PTR outBlocked (pcb_PTR p) {
	semd_PTR predecessor = searchSemd(p->p_semAdd) {
		/* Yay. Let outProcQ return successfully or report the error with NULL */
		return (outProcQ(&(predecessor->s_next->s_proQ), p));
	} else {
